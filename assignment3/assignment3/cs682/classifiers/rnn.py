...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type=='rnn':
            ...
            h, cache_rnn = lstm_forward(captions_in, h0, Wx, Wh, b)
        
        scores, cache_score = temporal_affine_forward(h, W_vocab, b_vocab)
        loss, dx = temporal_softmax_loss(scores, captions_out, mask)
        
        #Backward
        dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dx, cache_score)
    ...
    def sample(self, features, max_length=30):
        ...
        cur_h = features.dot(W_proj) + b_proj
        cur_c = np.zeros_like(cur_h)
        w_embed, _ = word_embedding_forward(self._start, W_embed)

        for i in range(max_length):
            if self.cell_type=='rnn':
                cur_h, cache = rnn_step_forward(w_embed, cur_h, Wx, Wh, b)
            elif self.cell_type=='lstm':
                cur_h, cur_c, cache = lstm_step_forward(w_embed, cur_h, cur_c, Wx, Wh, b)
                
            scores = np.dot(cur_h, W_vocab) + b_vocab
            captions[:, i] = np.argmax(scores, axis=1)
            cur_words = captions[:, i]
            w_embed, _ = word_embedding_forward(cur_words, W_embed)
